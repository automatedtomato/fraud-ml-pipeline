...
class TransactionRequest(BaseModel):
    ...
    trans_ts: str = Field(..., example="2023-07-08 15:00:00", description="Timestamp of the transaction")
    ...
    long: float = Field(..., example=-74.0060, description="Longitude of the transaction")
    city_pop: int = Field(..., example=8419000, description="Population of the transaction city")
    age_at_tx: int = Field(..., example=25, description="Age of the customer at the time of the transaction")
    hour_of_day: int = Field(..., example=14, description="Hour of the day the transaction occurred")
    day_of_week: int = Field(..., example=3, description="Day of the week the transaction occurred (0: Sunday)")
    distance_from_home_km: float = Field(..., example=5.2, description="Distance from the customer's home to the transaction location")
    time_since_last_tx_sec: Optional[float] = Field(0.0, example=3600.0, description="Time since the last transaction in seconds")
    tx_in_last_24h: int = Field(..., example=3, description="Number of transactions in the last 24 hours")
    ...
    tx_with_same_merch_in_last_1h: int = Field(..., example=2, description="Number of transactions with the same merchant in the last 1 hour")
...
class PredictionResponse(BaseModel):
    ...
    """
    fraud_score: float = Field(..., example=0.85, description="The predicted fraud score for the transaction")
    model_version: str = Field(..., example="pytorch_tabnet_v2", description="The version of the model used for prediction")
    risk_level: str = Field(..., example="high", description="The risk level of the transaction (categorical: low, medium, high)")
