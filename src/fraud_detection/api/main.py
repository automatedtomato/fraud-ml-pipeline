...
async def load_model():
    ...
    model_params = load_config(root="models").get("pytorch_tabnet_v1", {}).get("model_params", {})
    ...
    try:
        ...
        logger.error(f"Model not found: {model_path}. The API will not be able to make predictions.")
        model_assets["model"] = None


@app.get("/health", tags=["Health Check"])
async def health_check():
    return {"status": "OK", "message": "API is running healthy.", "serving_from": str(file_path)}
...
async def predict_fraud(transaction: TransactionRequest):
    ...
    if model_assets["model"] is None:
        raise HTTPException(status_code=503, detail="Model not loaded. The service is not available.")
    if not FEATURES:
        raise HTTPException(status_code=500, detail="Features not loaded. The service is not available.")
