...
def train(split_ratio: float = 0.8):
    ...
    logger.info(f"Total chunks: {total_chunks}, Train chunks: {train_chunks}, Validation chunks: {total_chunks - train_chunks}")
    logger.info(f"Total rows: {total_rows}, Train rows: {train_rows}, Validation rows: {total_rows - train_rows}")
    ...
    train_iterator = pd.read_sql_query(train_query, engine, chunksize=CHUNK_SIZE, parse_dates=['trans_ts'])
    ...
    for i, chunk_df in enumerate(train_iterator):
        ...
        logger.info(f"Memory usage optimized: {mem_before:.2f} MB -> {mem_after:.2f} MB")
    ...
    val_iterator = pd.read_sql_query(val_query, engine, chunksize=CHUNK_SIZE, parse_dates=['trans_ts'])
