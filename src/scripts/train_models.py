...
def _calc_scale_pos_weight(table_name: str, train_rows: int, target: str, engine: object) -> float:
    ...
    if pos_count == 0:
        logger.warinig("No positive samples in training data. Setting scale_pos_weight to 1.")
        return 1
...
def train(split_ratio: float = 0.8):
    ...
    for i, chunk_df in enumerate(val_iterator):
        ...
        for model_name, model in models_to_train.items():
            features = models_config[model_name]["features"]
    ...
    for model_name, y_pred_proba in all_preds.items():
        ...
        pr_auc = precision_recall_curve(y_true, y_pred_proba)
